...
namespace SharpML.Recurrent.Trainer
{
    public class Trainer
    {
        ...
        public static double train<T>(ref int trainingEpochs, double learningRate, INetwork network, DataSet data, int reportEveryNthEpoch, Random rng, Action<double, int> callback = null) where T : INetwork
        {
            return train<T>(ref trainingEpochs, learningRate, network, data, reportEveryNthEpoch, false, false, null, rng, callback);
        }

        public static double train<T>(ref int trainingEpochs, double learningRate, INetwork network, DataSet data, int reportEveryNthEpoch, bool initFromSaved, bool overwriteSaved, String savePath, Random rng, Action<double, int> callback = null) where T : INetwork
        {
            Console.WriteLine("--------------------------------------------------------------");
            if (initFromSaved)
            {
            ...
            for (int epoch = 0; epoch < trainingEpochs; epoch++)
            {
                ...
                {
                    data.DisplayReport(network, rng);
                    if (callback != null) callback(reportedLossTrain,epoch);
                }
